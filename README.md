...
[![Build Status](https://img.shields.io/github/actions/workflow/status/Cha0sThe0ry/chaos-modpack/build.yml)](https://github.com/Cha0sThe0ry/chaos-modpack/actions)
...
```bash
git clone https://github.com/Cha0sThe0ry/chaos-modpack
cd chaos-modpack
```

The `build.sh` script initializes the pinned `gmqcc` and Chaos `dev-chaos` data submodules, then compiles the mod. Output files are generated in `build/<branch>/`.
...
To play the mod locally, copy the `.dat` files from `build/<branch>/` into the userdir/data.
